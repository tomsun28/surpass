...
const roleRef = ref<InstanceType<typeof ElForm> | null>(null);
...
const title: any = ref("");
const dateRange: any = ref<any>([]);
const menuOptions: any = ref<any>([]);
const menuExpand: any = ref(false);
const menuNodeAll: any = ref(false);
const deptExpand: any = ref(true);
const deptNodeAll: any = ref(false);
const deptOptions: any = ref<any>([]);
const openDataScope: any = ref(false);
const menuRef: any = ref(undefined);
const deptRef: any = ref(undefined);
...
/** 角色状态修改 */
function handleStatusChange(row: any): any {
  let text: any = row.status === "0" ? "启用" : "停用";
...
/** 更多操作 */
function handleCommand(command: any, row: any): any {
  switch (command) {
...
/** 查询菜单树结构 */
function getMenuTreeselect(): any {
...
function getDeptAllCheckedKeys(): any {
  // 目前被选中的部门节点
  let checkedKeys: any = deptRef.value.getCheckedKeys();
  // 半选中的部门节点
  let halfCheckedKeys: any = deptRef.value.getHalfCheckedKeys();
...
function handleUpdate(row: any): any {
  ...
  const roleMenu: any = getRoleMenuTreeselect(roleId);
  getRole(roleId).then((response: any) =>  {
    ...
    nextTick(() => {
      roleMenu.then((res: any) => {
        let checkedKeys: any = res.checkedKeys;
...
/** 根据角色ID查询菜单树结构 */
function getRoleMenuTreeselect(roleId: any): any {
...
/** 根据角色ID查询部门树结构 */
function getDeptTree(roleId: any): any {
...
function getMenuAllCheckedKeys(): any {
  // 目前被选中的菜单节点
  let checkedKeys: any = menuRef.value.getCheckedKeys();
  // 半选中的菜单节点
  let halfCheckedKeys: any = menuRef.value.getHalfCheckedKeys();
  checkedKeys.unshift.apply(checkedKeys, halfCheckedKeys);
  return checkedKeys;
}
...
/** 选择角色权限范围触发 */
function dataScopeSelectChange(value: any): any {
  if (value !== "2") {
...
/** 分配数据权限操作 */
function handleDataScope(row: any): any {
  reset();
  const deptTreeSelect: any = getDeptTree(row.roleId);
