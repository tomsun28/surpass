...
import { listPost, addPost, deleteBatch, getPost, updatePost } from "@/api/system/post";

const {proxy} = getCurrentInstance()!;
const postRef = ref<InstanceType<typeof ElForm> | null>(null);
const { sys_normal_disable } = proxy.useDict("sys_normal_disable");

const postList: any = ref<any>([]);
  ...
  postRef?.value?.validate((valid: any) =>  {
    if (valid) {
      if (form.value.postId != undefined) {
        updatePost(form.value).then((response: any) =>  {
          modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addPost(form.value).then((response: any) =>  {
