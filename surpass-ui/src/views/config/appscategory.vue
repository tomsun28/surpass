  ...
  <div class="app-container">
    ...
    <el-card class="common-card">
      ...
      <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
        ...
        <el-table-column :label="$t('jbx.history.connectorDescription')" align="center" prop="description"
...
<script setup name="Config-appscategory" lang="ts">
...
import {
  getCategoryFetch,
  getCategoryFetchById,
  addCategoryFetch,
  editCategoryFetch,
  delCategoryFetch
} from "@/api/config/appscategory";
...
function getList(): any {
  loading.value = true;
  getCategoryFetch(queryParams.value).then((res: any) =>  {
    loading.value = false;
    if (res.code === 0) {
...
function handleUpdate(row: any): any {
  reset();
  const id: any = row.id || ids.value;
  getCategoryFetchById(id).then((res: any) =>  {
    form.value = res.data;
    open.value = true;
  ...
  formRef?.value?.validate((valid: any) =>  {
    if (valid) {
      if (form.value.id != undefined) {
        editCategoryFetch(form.value).then((response: any) =>  {
          modal.msgSuccess(t('jbx.alert.operate.success'));
          open.value = false;
          getList();
        });
      } else {
        addCategoryFetch(form.value).then((response: any) =>  {
          modal.msgSuccess(t('jbx.alert.operate.success'));
          open.value = false;
          getList();
        });
