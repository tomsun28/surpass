  ...
  const option: any = {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        name = fmtHourText(name) + "-" + fmtHourText(parseInt(name) + 1)
    ...
    xAxis: {
      type: 'category',
      data: dashboard.value.reportDayHour.map((t: any) => {
        return fmtHourText(t.reportName)
      })
    },
    yAxis: {
      type: 'value'
...
const myEcharts2 = () => {
  ...
  const option: any = {
    ...
    xAxis: {
      type: 'category',
      data: dashboard.value.reportMonth.map((t: any) => {
        return t.reportName
      })
    },
    yAxis: {
