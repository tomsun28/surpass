  ...
  <div class="app-container">
    <el-card class="common-card">
      <el-tabs v-model="activeName" class="demo-tabs" @tab-change="onGo">
        <el-tab-pane label="角色权限" name="access"></el-tab-pane>
      ...
      <el-form :model="queryParams" ref="queryRef" :inline="true" style="margin-bottom: 10px">
        <el-form-item :label="$t('jbx.roles.name') + '：'" prop="groupName">
          <el-input
      ...
      <el-row :gutter="20" style="min-height: 600px">
        <el-col :span="12">
          <el-table v-loading="loading" :data="list" highlight-current-row @row-click="changeRow">
            <el-table-column :label="$t('jbx.roles.name')" prop="roleName"/>
            <el-table-column :label="$t('jbx.roles.category.category')" align="center">
              <template #default="scope">
                <el-button type="text">
                  <dict-tag :options="group_category_options" :value="scope.row.category"></dict-tag>
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-col :span="12" style="margin-top: -40px;">
          ...
          <el-table v-loading="loading" :data="rightList" @selection-change="handleSelectionChange">
            ...
            <el-table-column :label="$t('jbx.text.action')" align="center"
              ...
              <template #default="scope">
                <el-button @click="handleVisible(scope.row,)" v-if="scope.row.visible === 0">{{
                    $t('jbx.text.display')
                  }}
                </el-button>
                <el-button @click="handleVisible(scope.row,)" v-if="scope.row.visible === 1">{{
                    $t('jbx.text.hide')
                  }}
                </el-button>
...
import NotAuthApps from "@/views/access/not-auth-apps/index.vue"
...
const activeName: any = ref("access")

const {group_category_options} = proxy.useDict("group_category_options");
...
//获取路由参数
const {groupId, groupName} = route.query;
...
function getRightList(): any {
  ...
  loading.value = true;
  apiAuthGroupApps(queryAuthParams.value).then((res: any) => {
    loading.value = false;
...
function handleVisible(row: any): any {
  ...
  apiUpdateVisibleAccess(data).then((res: any) => {
    modal.msgSuccess(t('jbx.alert.operate.success'));
    getRightList();
  });
}
...
function selectApps(ids: any): any {
  ...
  apiGroupAuthzApp(data).then((res: any) => {
    if (res.code === 0) {
      modal.msgSuccess(t('jbx.alert.operate.success'));
      getRightList();
    }
...
if (groupId != undefined) {
  leftObj.value = {}
  leftObj.value.id = groupId;
  leftObj.value.groupName = groupName;
  getRightList()
}
