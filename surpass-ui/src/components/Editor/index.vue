...
const uploadUrl: any = ref(import.meta.env.VITE_APP_BASE_API + "/common/upload"); // 上传的图片服务器地址
...
onMounted(() => {
  if (props.type == 'url') {
    let quill: any = quillEditorRef.value.getQuill();
    let toolbar: any = quill.getModule("toolbar");
...
function handleBeforeUpload(file: any): any {
  const type: any = ["image/jpeg", "image/jpg", "image/png", "image/svg"];
...
function handleUploadSuccess(res: any, file: any): any {
  ...
  if (res.code == 200) {
    ...
    // 获取光标位置
    let length: any = quill.selection.savedRange.index;
