...
const emit: any = defineEmits(['update'])
...
const radioValue: any = ref(2)
const cycle01: any = ref(2)
const cycle02: any = ref(3)
const average01: any = ref(1)
const average02: any = ref(2)
const weekday: any = ref(2)
const checkboxList: any = ref<any>([])
const checkCopy: any = ref([2])
...
const weekdayTotal: any = computed(() => {
    weekday.value = props.check(weekday.value, 1, 7)
    return weekday.value + 'L'
})
const checkboxString: any = computed(() => {
    return checkboxList.value.join(',')
})
watch(() => props.cron.week, (value: any) =>  changeRadioValue(value))
watch([radioValue, cycleTotal, averageTotal, weekdayTotal, checkboxString], () => onRadioChange())
function changeRadioValue(value: any): any {
    if (value === "*") {
        radioValue.value = 1
    } else if (value === "?") {
        radioValue.value = 2
    } else if (value.indexOf("-") > -1) {
        const indexArr: any = value.split('-')
        ...
        const indexArr: any = value.split("L")
        weekday.value = Number(indexArr[0])
        radioValue.value = 5
    } else {
        checkboxList.value = [...new Set(value.split(',').map((item: any) =>  Number(item)))]
        radioValue.value = 6
    }
}
function onRadioChange(): any {
