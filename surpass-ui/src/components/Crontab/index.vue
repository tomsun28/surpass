...
const emit: any = defineEmits(['hide', 'fill'])
...
const crontabValueString: any = computed(() => {
    const obj: any = crontabValueObj.value
    return obj.second
        + " "
        + obj.min
        + " "
...
// tab切换值
function tabCheck(index: any): any {
    tabActive.value = index
}
// 由子组件触发，更改表达式组成的字段值
function updateCrontabValue(name: any, value: any, from: any): any {
    crontabValueObj.value[name] = value
}
// 表单选项的子组件校验数字格式（通过-props传递）
function checkNumber(value: any, minLimit: any, maxLimit: any): any {
    // 检查必须为整数
...
// 隐藏弹窗
function hidePopup(): any {
    emit("hide")
}
// 填充表达式
function submitFill(): any {
    emit("fill", crontabValueString.value)
    hidePopup()
}
function clearCron(): any {
    // 还原选择项
    crontabValueObj.value = {
        second: "*",
        min: "*",
