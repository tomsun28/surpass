...
const dayRule: any = ref('')
const dayRuleSup: any = ref('')
const dateArr: any = ref<any>([])
const resultList: any = ref<any>([])
const isShow: any = ref(false)
watch(() => props.ex, () => expressionChange())
// 表达式值变化时，开始去计算结果
function expressionChange(): any {
    // 计算开始-隐藏结果
    isShow.value = false;
    // 获取规则数组[0秒、1分、2时、3日、4月、5星期、6年]
    let ruleArr: any = props.ex.split(' ');
    // 用于记录进入循环的次数
    let nums: any = 0;
    // 用于暂时存符号时间规则结果的数组
    let resultArr: any = [];
    // 获取当前时间精确至[年、月、日、时、分、秒]
    let nTime: any = new Date();
    let nYear: any = nTime.getFullYear();
    let nMonth: any = nTime.getMonth() + 1;
    let nDay: any = nTime.getDate();
    let nHour: any = nTime.getHours();
    let nMin: any = nTime.getMinutes();
    let nSecond: any = nTime.getSeconds();
    ...
    // 重置月日时分秒的函数(后面用的比较多)
    const resetSecond: any = function () {
        sIdx = 0;
        nSecond = sDate[sIdx]
    }
    const resetMin: any = function () {
        mIdx = 0;
        nMin = mDate[mIdx]
        resetSecond();
    }
    const resetHour: any = function () {
        hIdx = 0;
        nHour = hDate[hIdx]
        resetMin();
    }
    const resetDay: any = function () {
        DIdx = 0;
        nDay = DDate[DIdx]
        resetHour();
    }
    const resetMonth: any = function () {
        MIdx = 0;
        nMonth = MDate[MIdx]
        resetDay();
    }
    ...
    // 循环年份数组
    goYear: for (let Yi: any = YIdx; Yi < YDate.length; Yi++) {
        let YY: any = YDate[Yi];
        // 如果到达最大值时
        ...
        // 循环月份数组
        goMonth: for (let Mi: any = MIdx; Mi < MDate.length; Mi++) {
            // 赋值、方便后面运算
            let MM: any = MDate[Mi];
            MM = MM < 10 ? '0' + MM : MM;
            // 如果到达最大值时
            ...
            // 循环日期数组
            goDay: for (let Di: any = DIdx; Di < DDate.length; Di++) {
                // 赋值、方便后面运算
                let DD: any = DDate[Di];
                ...
                DD = DD < 10 ? '0' + DD : DD;
                // 循环“时”数组
                goHour: for (let hi: any = hIdx; hi < hDate.length; hi++) {
                    let hh: any = hDate[hi] < 10 ? '0' + hDate[hi] : hDate[hi]
                    // 如果到达最大值时
                    ...
                    // 循环"分"数组
                    goMin: for (let mi: any = mIdx; mi < mDate.length; mi++) {
                        let mm: any = mDate[mi] < 10 ? '0' + mDate[mi] : mDate[mi];
                        // 如果到达最大值时
...
// 获取"年"数组
function getYearArr(rule: any, year: any): any {
...
// 获取"月"数组
function getMonthArr(rule: any): any {
...
// 获取"日"数组-主要为日期规则
function getWeekArr(rule: any): any {
    ...
    if (dayRule.value === '' && dayRuleSup.value === '') {
        if (rule.indexOf('-') >= 0) {
            ...
            let matchRule: any = rule.match(/[0-9]{1}/g);
...
// 获取"日"数组-少量为日期规则
function getDayArr(rule: any): any {
...
// 获取"时"数组
function getHourArr(rule: any): any {
...
// 获取"分"数组
function getMinArr(rule: any): any {
...
// 获取"秒"数组
function getSecondArr(rule: any): any {
...
function getOrderArr(min: any, max: any): any {
    let arr: any = [];
...
// 根据规则中指定的零散值返回一个数组
function getAssignArr(rule: any): any {
    let arr: any = [];
    let assiginArr: any = rule.split(',');
    for (let i: any = 0; i < assiginArr.length; i++) {
...
// 根据一定算术规则计算返回一个数组
function getAverageArr(rule: any, limit: any): any {
    let arr: any = [];
    let agArr: any = rule.split('/');
    let min: any = Number(agArr[0]);
    let step: any = Number(agArr[1]);
    while (min <= limit) {
        arr.push(min);
        min += step;
    }
...
// 根据规则返回一个具有周期性的数组
function getCycleArr(rule: any, limit: any, status: any): any {
    // status--表示是否从0开始（则从1开始）
    let arr: any = [];
    let cycleArr: any = rule.split('-');
    let min: any = Number(cycleArr[0]);
    let max: any = Number(cycleArr[1]);
    if (min > max) {
        max += limit;
    }
    for (let i: any = min; i <= max; i++) {
...
// 比较数字大小（用于Array.sort）
function compare(value1: any, value2: any): any {
    if (value2 - value1 > 0) {
        return -1;
    } else {
        return 1;
...
function formatDate(value: any, type: any): any {
    // 计算日期相关值
    let time: any = typeof value == 'number' ? new Date(value) : value;
