...
public class DataSourceController {
    ...
    @GetMapping
    public Message<List<DataSource>> list() {
        List<DataSource> dataSources = dataSourceService.findAll();
        return Message.ok(dataSources);
    }

    @GetMapping("/{id}")
    public Message<DataSource> getById(@PathVariable String id) {
    ...
    @PutMapping("/{id}")
    public Message<String> update(@PathVariable String id, @Valid @RequestBody DataSource dataSource) {
        dataSource.setId(id);
    ...
    @PostMapping("/test")
    public Message<Boolean> testConnection(@Valid @RequestBody DataSource dataSource) {
        boolean success = dataSourceService.testConnection(dataSource);
        return Message.ok(success);
    }
