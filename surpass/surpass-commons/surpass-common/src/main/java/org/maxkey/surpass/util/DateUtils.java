...
package org.maxkey.surpass.util;
...
public class DateUtils {
	...
	private static final long MILLISECOND_ONE_MINUTE = 60 * 1000;

	private static final long MILLISECOND_ONE_HOUR = 3600 * 1000;

	private static final long MILLISECOND_ONE_DAY = MILLISECOND_ONE_HOUR * 24;
	...
	public static boolean compareTime(String endDate,Date startDate,String datePattern) {
		return DateUtils.parse(endDate, datePattern).before(startDate);
	}
	 ...
	 * @return
	 */
	public static int getDayOfWeek(final int SUN_FST_DAY_OF_WEEK) {
	...
	public static LocalDate lastDay(LocalDate day) {
		int year = day.getYear(); // 指定的年份
        int month = day.getMonthValue();   // 指定的月份
	...
	public static DateTime toUtcDate(String date){
		return new DateTime(date, ISOChronology.getInstanceUTC());
	}
