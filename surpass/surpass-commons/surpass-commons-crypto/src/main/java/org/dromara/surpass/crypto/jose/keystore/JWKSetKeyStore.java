...
package org.dromara.surpass.crypto.jose.keystore;
...
import org.dromara.surpass.crypto.RSAUtils;
...
public class JWKSetKeyStore {
    private static final  Logger logger = LoggerFactory.getLogger(JWKSetKeyStore.class);
        ...
        if (jwkSet == null) {
            if (location != null) {
                ...
                if (location.exists() && location.isReadable()) {
                    ...
                    try {
                        logger.debug("JWK location " + location.getURL());
		...
		if(StringUtils.isNotBlank(mediaType) && mediaType.equalsIgnoreCase("XML")) {
			...
			for(JWK jwk : jwkSet.getKeys()) {
				...
				try {
					...
					logger.error("JOSEException ", mediaType);
				}
			}
