...
public interface AuthResourceDao extends JpaRepository<AuthResource, Long> {
    ...
    @Query("select CONCAT(LOWER(resource.uri),'===', resource.method) " +
            "from AuthResource resource where resource.status = 9 order by resource.id")
    ...
    @Query("select distinct resource from AuthResource resource " +
            "left join AuthRoleResourceBind bind on bind.resourceId = resource.id " +
            "where bind.roleId = :roleId " +
            "order by resource.id asc")
    Page<AuthResource> findRoleOwnResource(@Param("roleId") Long roleId, Pageable request);
    ...
    @Query("select distinct resource from AuthResource resource " +
            ...
            "(select distinct bind.resourceId from AuthRoleResourceBind bind where bind.roleId = :roleId) " +
            "order by resource.id asc ")
    Page<AuthResource> findRoleNotOwnResource(@Param("roleId") Long roleId, Pageable request);
}
