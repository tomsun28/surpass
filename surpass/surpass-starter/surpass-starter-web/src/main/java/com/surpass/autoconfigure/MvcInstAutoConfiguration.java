...
import com.surpass.web.WebHttpInstRequestFilter;
...
public class MvcInstAutoConfiguration implements WebMvcConfigurer {
    private static final  Logger logger = LoggerFactory.getLogger(MvcInstAutoConfiguration.class);

    @Bean
    FilterRegistrationBean<Filter> webHttpInstRequestFilter(
                                                InstitutionsService institutionsService,
                                                ApplicationConfig applicationConfig) {
        logger.debug("WebHttpInstRequestFilter init for /* ");
